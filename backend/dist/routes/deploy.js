...
const deploymentSaveLocks = new Set();
...
async function authenticateRequest(req) {
    ...
    return (0, supabase_1.verifySupabaseJwt)(token);
}
async function waitForDeploymentCompletion(deploymentId, timeoutMs, pollIntervalMs = 1000) {
    const started = Date.now();
    while (Date.now() - started < timeoutMs) {
        const execution = await orchestrator_1.flowOrchestrator.getExecution(deploymentId);
        if (execution && (execution.status === types_1.FlowStatus.COMPLETED || execution.status === types_1.FlowStatus.FAILED)) {
            return execution;
        }
...
function extractDeploymentUrl(execution) {
...
function extractVercelUrlFromText(text) {
    const match = text.match(/https?:\/\/[a-zA-Z0-9][a-zA-Z0-9.-]*\.vercel\.app(?:\/[^\s"'`<>]*)?/i);
    return match?.[0];
}
async function extractVercelUrlFromPipeline(execution) {
    ...
    try {
        ...
        for (const job of orderedJobs) {
            ...
            const logs = await gitlab_adapter_1.gitlabAdapter.getJobLogs(job.id);
        ...
        console.warn(`[Deploy] Unable to extract Vercel URL from pipeline logs: ${error.message}`);
    }
    return undefined;
}
async function resolveDeploymentUrl(execution) {
...
function isTerminalExecution(execution) {
    return execution.status === types_1.FlowStatus.COMPLETED || execution.status === types_1.FlowStatus.FAILED;
}
async function persistDeploymentIfNeeded(deploymentId, userId, execution) {
    ...
    try {
        ...
        const saved = await (0, supabase_1.saveDeploymentRecord)({
            userId,
            url: deploymentUrl,
            status: execution.status === types_1.FlowStatus.COMPLETED ? 'success' : 'failed'
        });
...
exports.deployRouter.post('/', async (req, res) => {
    try {
        ...
        const { environment, version, branch, parameters, waitForCompletion } = req.body;
        ...
        deployment_tracker_1.deploymentTracker.initializeDeployment(deploymentId);
        ...
        orchestrator_1.flowOrchestrator.executeFlow(deploymentId, 'deploy-flow', {
            environment: targetEnvironment,
        ...
        const status = terminalExecution.status === types_1.FlowStatus.COMPLETED ? 'success' : 'failed';
        const savedDeployment = await (0, supabase_1.saveDeploymentRecord)({
            userId: user.id,
            url: deploymentUrl,
            status
        });
        deploymentOwners.set(deploymentId, { userId: user.id, saved: true });
        if (terminalExecution.status !== types_1.FlowStatus.COMPLETED) {
            res.status(502).json({
                deploymentId,
                status: 'failed',
        ...
        const message = error.message || 'Deployment request failed';
...
exports.deployRouter.get('/status/:id', async (req, res) => {
    try {
        ...
        const deploymentStatus = deployment_tracker_1.deploymentTracker.getDeploymentStatus(id, execution);
        ...
        const message = error.message || 'Unable to fetch deployment status';
        const statusCode = message.includes('Authorization') || message.includes('token') ? 401 : 500;
        res.status(statusCode).json({ error: message });
    }
});
// Get deployment history for the authenticated user
exports.deployRouter.get('/history', async (req, res) => {
    try {
        const user = await authenticateRequest(req);
        const deployments = await (0, supabase_1.getDeploymentsForUser)(user.id);
        res.json({ deployments });
    }
    catch (error) {
        const message = error.message || 'Unable to load deployment history';
        const statusCode = message.includes('Authorization') || message.includes('token') ? 401 : 500;
        res.status(statusCode).json({ error: message });
    }
});
...
exports.deployRouter.post('/cancel/:id', async (req, res) => {
    try {
        ...
        const message = error.message || 'Unable to cancel deployment';
        const statusCode = message.includes('Authorization') || message.includes('token') ? 401 : 500;
        res.status(statusCode).json({ error: message });
    }
});
